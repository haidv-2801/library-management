...
namespace TOE.TOEIC.ApplicationCore.Interfaces
{
    public class UserService : BaseService<User>, IUserService
    {
        ...
        public ServiceResult GetUsersFilterPaging(string filterValue, int pageSize, int pageNumber)
        {
            //.1 Lấy danh sách nhân viên phân trang
            var employeeFilterPaging = _userRepository.GetUsersFilterPaging(filterValue, pageSize, pageNumber).ToList();

            //2. Lấy tổng số bản ghi
            int totalRecord = _userRepository.GetTotalUserFilter(filterValue);
        ...
        /// <returns></returns>
        public object Login(User userRequest)
        {
            ...
            var user = new User
            {
                UserID = Guid.Parse("b69e90d7-324c-4d54-9da8-38ba0ba02967"),
                UserName = "dovanhai",
            ...
            else
            {
                ...
                {
                    return new { userInfo = JsonConvert.SerializeObject(new UserDTO { Email = user.Email, UserName = user.UserName }), token = GenerateJSONWebToken(user) };
                }
            }
            return null;
        }
        ...
        //        _serviceResult.MISACode = TOECode.InValid;
        ...
        private object GetValueByProperty(UserDTO user, string propName)
        {
            var propertyInfo = user.GetType().GetProperty(propName);
            ...
            return propertyInfo.GetValue(user, null);
        }
        #endregion
    }
}
